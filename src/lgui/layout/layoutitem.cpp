...
lgui::SizeConstraint lgui::LayoutItem::get_horz_constraint(lgui::SizeConstraint wc) const {
    ...
    SizeConstraintMode mode = (malign.is_set(Align::HStretch) &&
                               (malign.is_set(Align::HTakeAll) || wc.mode() == SizeConstraintMode::Exactly))
...
lgui::SizeConstraint lgui::LayoutItem::get_vert_constraint(lgui::SizeConstraint hc) const {
    ...
    SizeConstraintMode mode = (malign.is_set(Align::VStretch) &&
                               (malign.is_set(Align::VTakeAll) || hc.mode() == SizeConstraintMode::Exactly))
                              ? SizeConstraintMode::Exactly
                              : SizeConstraintMode::Maximum;
