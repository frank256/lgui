...
void MouseTrackHelper::register_drag_entered(Widget* widget, Position mouse_pos, int button,
                                             DragRepresentation* drag_repr, double timestamp)
{
    for (Widget* umw = widget; umw != nullptr; umw = umw->parent()) {
        if (!contains(mwidgets_under_drag, umw)) {
            ...
            if (mdistr.distribute_dragdrop_event(umw, DragDropEvent::Entered, timestamp,
                                                 mouse_pos, button, drag_repr, true)) {
    ...
    if (drag_repr->target_widget()) {
        mdistr.distribute_dragdrop_event(drag_repr->target_widget(), DragDropEvent::Moved, timestamp,
                                         mouse_pos, mlast_mouse_state.dragged_button(), drag_repr, true);
    }
}
    ...
    if (under_mouse) {
        ...
        if (!mdrag_repr) {
            ...
            register_drag_entered(under_mouse, mouse_pos, mlast_mouse_state.dragged_button(),
                                  mdrag_repr, mlast_mouse_state.timestamp());
...
void MouseTrackHelper::remove_not_under_mouse(Position mouse_pos, double timestamp)
{
    ...
    mwidgets_under_mouse.erase(std::remove_if(mwidgets_under_mouse.begin(), mwidgets_under_mouse.end(),
                                              [mouse_pos, timestamp, this](Widget* w) -> bool {
...
void MouseTrackHelper::prepare_drag_drop_operation(DragRepresentation* drag_repr, Position mouse_pos)
{
    ASSERT(drag_repr);
...
void MouseTrackHelper::finish_drag_drop_operation(Position mouse_pos, int button, double timestamp)
{
    ASSERT(mdrag_repr);
...
void MouseTrackHelper::abort_drag(bool send_events, bool send_dd_end_to_gone_src)
{
    ...
    if (src) {
        if (send_dd_end_to_gone_src) {
            mdistr.distribute_dragdrop_event(src, DragDropEvent::DragEnded, mlast_mouse_state.timestamp(),
                                             mlast_mouse_state.pos(), mlast_mouse_state.dragged_button(),
                                             mdrag_repr, true);
            mdistr.distribute_mouse_event(src, MouseEvent::Released, mlast_mouse_state.timestamp(),
                                          mlast_mouse_state.pos(), mlast_mouse_state.dragged_button(), true);
        }
    ...
    if (send_events) {
        ...
        for (Widget* w : mwidgets_under_mouse) {
            mdistr.distribute_mouse_event(w, MouseEvent::Left, mlast_mouse_state.timestamp(),
